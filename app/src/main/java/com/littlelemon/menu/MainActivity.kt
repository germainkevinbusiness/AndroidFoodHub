...
class MainActivity : ComponentActivity() {
    ...
    fun InitUI() {
        ...
        ProductsGrid(products = products,
            startProductActivity = {
                startProductActivity(productItem = it)
            })
    }
    ...
    private fun startProductActivity(productItem: ProductItem) {
        ...
        Intent(this, ProductActivity::class.java).apply {
            putExtra(PRODUCT_TITLE, productItem.title)
            putExtra(PRODUCT_PRICE, productItem.price)
            putExtra(PRODUCT_CATEGORY, productItem.category)
            putExtra(PRODUCT_IMAGE, productItem.image)
            startActivity(this)
        }
    }
